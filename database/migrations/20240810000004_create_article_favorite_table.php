...
class CreateArticleFavoriteTable extends Migrator
{
    /**
     * Change Method.
     *
    ...
    public function down()
    {
        ...
        // 从blog_post表中删除favorites字段（如果存在）
        if ($this->hasTable($prefix.'blog_post')) {
            $table = $this->table($prefix.'blog_post');

            // 检查字段是否存在
            if ($table->hasColumn('favorites')) {
                // 删除favorites字段
                $table->removeColumn('favorites')->update();
                $this->output->writeln('成功从 blog_post 表中删除 favorites 字段');
            } else {
                $this->output->writeln('favorites 字段不存在于 blog_post 表中');
            }
        } else {
            $this->output->writeln('blog_post 表不存在');
        }
    }
}
